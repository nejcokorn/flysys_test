...
`docs/digikey-bom.md`. `J2` debug pads are copper features on the PCB, not a
purchased connector, so they are intentionally excluded from the purchasing
BOM.
...
| `SW1` | `user_button` | `KMR211NG LFS` | Active-low user/BOOT button | Accessible edge area. Shares `BOOT_USER_BTN_N`, so enclosure should avoid accidental boot-mode holds. |
...
| `J2` | `debug` | 1x6 debug pads | GND, `+3V3`, `EN`, `BOOT`, UART0 TX/RX | Near MCU, reachable during bring-up. Not a production connector. |
...
| `C1` | `c_bat_bulk` | 10 uF | `BAT` to `GND` | Battery rail bulk. Place near `J1` and `U1 BAT`. |
...
| `R6` | `r_boot_pullup` | 10 k | ESP32-S3 `IO0`/BOOT/user-button pull-up. Place near `U4` and `SW1`. |
...
- `+3V3` fans out from `U3` to `U4`, `U5`, `U2`, pull-ups, and debug pads.
...
The local footprints for `J1`, `BZ1`, `Q1`, `Q2`, `SW1`, and `J2` are draft local footprints. Before production, verify each against manufacturer land-pattern drawings and enclosure mechanics.
